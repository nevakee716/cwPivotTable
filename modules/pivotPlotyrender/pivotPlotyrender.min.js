    ...
    makePlotlyChart = function (hovermode, traceOptions, layoutOptions, transpose) {
      ...
      return function (pivotData, opts) {
        ...
        layout = {
          title: titleText,
          hovermode: hovermode,
          width: document.querySelector(".pvtRendererArea").offsetWidth,
          height: document.querySelector(".pvtRendererArea").offsetHeight,
          margin: {
            l: isInDisplay ? 20 : 0,
            r: isInDisplay ? 20 : 0,
            b: isInDisplay ? 20 : 0,
        ...
        if (traceOptions.type === "bar") {
          ...
          data.forEach(function (d, i) {
            d.text = d[traceOptions.orientation === "h" ? "x" : "y"].map(function (y, index) {
              return Math.round((y * 100) / totalByX[d[traceOptions.orientation === "h" ? "y" : "x"][index]], 3) + "%";
            });
            d.textposition = "auto";
            d.hovertemplate = traceOptions.orientation === "h" ? "<b>%{text}<br>%{x}</b>" : "<b>%{text}<br>%{y}</b>";
          });
        ...
        if (traceOptions.type === "pie") {
          data.forEach(function (d, i) {
            d.marker = {
              colors: d.labels.map(opts.plotlyConfig.getColor.bind(null, hAxisTitle)),
            };
          });
        } else if (traceOptions.type === "bar") {
          data.forEach(function (d, i) {
            d.marker = {
              color: opts.plotlyConfig.getColor(groupByTitle, d.name),
            };
          });
        }
