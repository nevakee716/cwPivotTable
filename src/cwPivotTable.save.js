...
(function(cwApi, $) {
    ...
    cwPivotTable.prototype.directSaveNewNetwork = function(pivotName) {
        ...
        cwAPI.CwEditSave.setPopoutContentForGrid(cwApi.CwPendingChangeset.ActionType.Create, null, newObj, 0, this.definition.capipivotScriptname, function(elem) {
            if (elem && elem.status == "Ok") {
                if(!cwAPI.isIndexPage()) {
                    ...
                    cwAPI.CwEditSave.setPopoutContentForGrid(cwApi.CwPendingChangeset.ActionType.Update, newObj, newNewObj, newObj.object_id, self.definition.capipivotScriptname, function(response) {
                        if (!cwApi.statusIsKo(response)) {
                            ...
                            $('.selectPivotConfiguration_' + self.nodeID).append(html)
                                .selectpicker("refresh");

                            $("div.selectPivotConfiguration_" + self.nodeID + " > option").remove();
                            $('.selectPivotConfiguration_' + self.nodeID).val(pivotName).selectpicker("refresh");
                        }
                    });
                } else {
